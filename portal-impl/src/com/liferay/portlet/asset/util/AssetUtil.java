...
public class AssetUtil {
	...
	public static String getCategoryPath(
			AssetCategory category, boolean containsActualCategoryName,
		...
		String separator = StringPool.SPACE + StringPool.GREATER_THAN +
			StringPool.SPACE;
		...
		if ((names.size() > 0) &&
				(names.get(names.size() - 1).equals(separator))) {
	...
	public static String getCategoryPath(
			long categoryId, boolean containsActualCategoryName,
			boolean containsVocabularyName, Locale locale)
		throws PortalException, SystemException {
		...
		return getCategoryPath(
			category, containsActualCategoryName, containsVocabularyName,
			locale);
	}
