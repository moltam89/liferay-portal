...
boolean autoDisplayDate = ParamUtil.getBoolean(request, "autoDisplayDate", true);

if (entry != null) {
	autoDisplayDate = false;
}
%>
	...
	<aui:fieldset>
		...
		<c:choose>
			<c:when test="<%= entry == null %>">
				...
				<aui:input label="display-date-will-be-based-on-create-date" name="autoDisplayDate" onClick="<%= autoDisplayDateOnClick %>" type="checkbox" value="<%= autoDisplayDate %>" />
			</c:when>
		</c:choose>
