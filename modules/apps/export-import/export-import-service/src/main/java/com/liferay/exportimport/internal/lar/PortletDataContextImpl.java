...
public class PortletDataContextImpl implements PortletDataContext {
	...
	public Element getImportDataElement(
		...
		ImportDataElementThreadLocal importDataElementThreadLocal =
			ImportDataElementThreadLocal.getImportDataElementThreadLocal();

		Element importDataElement = null;
		String key = StringPool.BLANK;

		if (importDataElementThreadLocal.isEnabled()) {
			key = ImportDataElementThreadLocal.getKey(
				_importDataRootElement, name, attribute, value);

			importDataElement = importDataElementThreadLocal.getElement(key);
		...
		if (importDataElementThreadLocal.isEnabled()) {
			importDataElementThreadLocal.put(key, importDataElement);
		}
	...
	public void setImportDataElementCacheEnabled(boolean enabled) {
		...
		importDataElementThreadLocal.setEnabled(enabled);

		if (!enabled) {
			importDataElementThreadLocal.clear();
		}
	}
	...
	protected Element getImportDataGroupElement(String name) {
		...
		if (importDataElementThreadLocal.isEnabled()) {
			...
			groupElement = importDataElementThreadLocal.getElement(key);
		...
		if (importDataElementThreadLocal.isEnabled()) {
			importDataElementThreadLocal.put(key, groupElement);
		}
